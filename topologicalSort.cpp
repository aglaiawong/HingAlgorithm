...
{ 
    ...
    list<int> *adj; 	//list in cpp as doubly linked list; no random access; constant time insert & removal
...
{ 
    ...
    Stack.push(v); 	//the top of stack = the first node in topological order 
} 
...
{ 
	...
	// this visited differs from each starting node
	// a topological sorting is found iff there exists a node 
	// whose path contains all nodes in graph
	// otherwise, flushes the entire visited[] by re-initialize
	...
	// here: iff all visit[i] are true; i.e. there exists a single starting node,
	// from where all nodes could be reached sequentially
